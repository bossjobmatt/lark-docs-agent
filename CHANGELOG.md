...
- **体验优化**：生成期间发送键变「⏹ 停止」（经 AbortController 中止，服务端同步中止上游请求）；流式期间用 vendored marked 增量渲染（节流 ~90ms + 轻量消毒），done 后仍替换为服务端权威渲染；自动滚动节流（用户上翻暂停跟随）；typing 指示随事件阶段更新；失败气泡带「↻ 重试」；删除会话二次确认；等待期间输入框保持可编辑。
