...
/** chat 系列路由共用的请求体解析：放宽上限读取 + 图片归一化校验；非法输入抛带 status 的错误（路由层转 400） */
async function parseChatBody(req) {
  ...
  if (!message && !images.length) {
    const err = new Error("message 或 images 至少其一不能为空");
    err.status = 400;
    throw err;
  }
  return { message, images, sessionId: body.sessionId };
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/chat") {
      let message, images, sessionId;
      try {
        ({ message, images, sessionId } = await parseChatBody(req));
      } catch (e) {
...
function describeLarkStatus(lark) {
  if (lark.available) {
    return lark.source === "env" ? `${lark.path}（LARK_CLI 指定，已通过安装与登录检测）` : `PATH 中的 lark-cli（已安装并登录）`;
  }
  if (lark.source === "env") {
    return `LARK_CLI 指定的 ${lark.path} 未通过检测（--version / auth status --json --verify）——修复后无需重启，检测会自动重试`;
  }
  return "未检测到已安装并登录的 lark CLI —— 解析飞书文档需要本地安装 lark-cli 并完成登录（LARK_CLI 或 PATH），检测失败后会自动重试";
}
